...
contract POI
{
  ...
  function setPubKey(bytes32 name, string key)  public  onlyOwner  payable
  {
    require(bytes(pubKeys[name]).length==0);
    pubKeys[name]=key;
  }

  function invalidate(bytes32 docHash) public onlyOwner  payable
  {
  ...
  function setRoot(bytes32 root, bytes32 name)public only Owner payable
  {
