...
contract Certification {
    ...
    event certificateLocked(bytes32 _certificateId);
    event certificateUnlocked(bytes32 _certificateId);
    ...
    function lock(string memory _id) public onlyOwner {
        bytes32 byte_id = stringToBytes32(_id);
        ...
        emit certificateLocked(byte_id);
    }

    function unlock(string memory _id) public onlyOwner {
        bytes32 byte_id = stringToBytes32(_id);
        require(certificates[byte_id].expiration_date != 0, "Certificate does not exist");
        require(certificates[byte_id].isLocked, "Certificate is not locked");
        certificates[byte_id].isLocked = false;
        emit certificateUnlocked(byte_id);
    }

    function getData(string memory _id) public view returns(string memory, string memory, string memory, uint256, string memory, bool) {
        bytes32 byte_id = stringToBytes32(_id);
        Certificate memory temp = certificates[byte_id];
        require(temp.expiration_date != 0, "No data exists");
        return (temp.candidate_name, temp.org_name, temp.course_name, temp.expiration_date, temp.ipfsLink, temp.isLocked);
    }
}
